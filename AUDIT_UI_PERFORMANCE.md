...
- **Práce s daty:** 7/10 — DataStore/Room/Flow použity správně, ale fallbackToDestructiveMigration může způsobit ztrátu dat.
...
- **IO v Composable souborech:** Některé stahování a diskové operace se spouštějí přímo z Composable/Activity s `Dispatchers.IO`. Návrh: přesunout do `ViewModel`/`Repository` a vystavit průběh přes `StateFlow`/`SharedFlow`.
...
- **DB migrace:** `fallbackToDestructiveMigration()` v DatabaseModule vede k výmazu dat při změně schématu. Doporučeno: připravit skutečné migrace nebo verzování.
...
- **Database DI (migrations):** [app/src/main/java/com/tobiso/tobisoappnative/di/DatabaseModule.kt](app/src/main/java/com/tobiso/tobisoappnative/di/DatabaseModule.kt#L21)
...
- **Refactor C:** Implementovat jen několik Room migrací místo `fallbackToDestructiveMigration`.
